            ...
            <div class="panel">
                <div class="panel-heading">
                    <div class="panel-control">
                        <a href="{{ route('backend.posts.create') }}" class="btn btn-primary btn-sm">Add Post</a>
                    </div>
                ...
                <div class="panel-body">
                    <table id="demo-dt-basic" class="table table-striped table-bordered" cellspacing="0" width="100%">
                        ...
                        @foreach($posts as $key => $post)
                            ...
                            <tr>
                                <td>{{$index}}</td>
                                <td>{{$post->$title_var ?? $title_var2}}</td>
                                <td>
                                    {{ substr(strip_tags($post->$description_var ?? $description_var2),0,200) }} ...
                                </td>
                                <td>{{date('Y-m-d',strtotime($post->created_at))}}</td>
                                <td>
                                    <a class="btn btn-primary btn-xs" href="{{route('backend.posts.edit',$post->id)}}">Edit</a>
                                    <button type="button" class="btn btn-danger btn-xs" data-toggle="modal" data-target="#exampleModal{{$post->id}}">
                                        Delete
                                    </button>
                                </td>
                            </tr>
