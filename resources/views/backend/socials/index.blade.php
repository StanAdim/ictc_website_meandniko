            ...
            <div class="panel">
                <div class="panel-heading">
                    <div class="panel-control">
                        <a href="{{ route('backend.socials.create') }}" class="btn btn-primary btn-sm">Add Social</a>
                    </div>
                    ...
                    <table id="demo-dt-basic" class="table table-striped table-bordered" cellspacing="0" width="100%">
                        ...
                        @foreach($socials as $key => $social)
                            ...
                            <tr>
                                ...
                                <td>{{$social->type}}</td>
                                <td>{{$social->link}}</td>
                                <td>{{date('Y-m-d',strtotime($social->created_at))}}</td>
                                <td>
                                    <a class="btn btn-primary btn-xs" href="{{route('backend.socials.edit',$social->id)}}">Edit</a>
                                    <button type="button" class="btn btn-danger btn-xs" data-toggle="modal" data-target="#exampleModal{{$social->id}}">
                                        Delete
                                    </button>
                                </td>
                            </tr>
                            ...
                            <div class="modal fade" id="exampleModal{{$social->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                <div class="modal-dialog" role="document">
                                    <form method="POST" action="{{route('backend.socials.destroy', $social->id)}}">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h5 class="modal-title" id="exampleModalLabel{{$social->id}}">Delete</h5>
