            ...
            <div class="panel">
                <div class="panel-heading">
                    <div class="panel-control">
                        <a href="{{ route('backend.sections.create') }}" class="btn btn-primary btn-sm">Add New Section</a>
                    </div>
                    ...
                    <table id="demo-dt-basic" class="table table-striped table-bordered" cellspacing="0" width="100%">
                        ...
                        @foreach($sections as $key => $section)
                            ...
                            <tr>
                                <td>{{$index}}</td>
                                <td>{{$section->slug}}</td>
                                <td>{{$section->$title_var ?? $title_var2}}</td>
                                <td>{{date('Y-m-d',strtotime($section->created_at))}}</td>
                                <td>
                                    <a class="btn btn-primary btn-xs" href="{{route('backend.sections.edit',$section->id)}}">Edit</a>
                                    <button type="button" class="btn btn-danger btn-xs" data-toggle="modal" data-target="#exampleModal{{$section->id}}">
                                        Delete
                                    </button>
                                </td>
                            </tr>
                            ...
                            <div class="modal fade" id="exampleModal{{$section->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                <div class="modal-dialog" role="document">
                                    <form method="POST" action="{{route('backend.sections.destroy', $section->id)}}">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h5 class="modal-title" id="exampleModalLabel{{$section->id}}">Delete</h5>
