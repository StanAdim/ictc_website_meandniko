            ...
            <div class="panel">
                <div class="panel-heading">
                    <div class="panel-control">
                        <a href="{{ route('backend.investments.create') }}" class="btn btn-primary btn-sm">Add Investment</a>
                    </div>
                ...
                <div class="panel-body">
                    <table id="demo-dt-basic" class="table table-striped table-bordered" cellspacing="0" width="100%">
                        ...
                        @foreach($investments as $key => $investment)
                            ...
                            <tr>
                                <td>{{$index}}</td>
                                <td>{{$investment->$title_var ?? $title_var2}}</td>
                                <td>
                                    {{ substr(strip_tags($investment->$description_var ?? $description_var2),0,200) }} ...
                                </td>
                                <td>{{date('Y-m-d',strtotime($investment->created_at))}}</td>
                                <td>
                                    <a class="btn btn-primary btn-xs" href="{{route('backend.investments.edit',$investment->id)}}">Edit</a>
                                    <button type="button" class="btn btn-danger btn-xs" data-toggle="modal" data-target="#exampleModal{{$investment->id}}">
                                        Delete
                                    </button>
                                </td>
                            </tr>
                            ...
                            <div class="modal fade" id="exampleModal{{$investment->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                <div class="modal-dialog" role="document">
                                    <form method="POST" action="{{route('backend.investments.destroy', $investment->id)}}">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h5 class="modal-title" id="exampleModalLabel{{$investment->id}}">Delete</h5>
                                                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                                    <span aria-hidden="true">&times;</span>
                                                </button>
                                            </div>
                                            <div class="modal-body">
                                                Are you sure you want to delete this investments?
                                            </div>
