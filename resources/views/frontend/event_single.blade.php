@extends('frontend.layout.main', ['title' => 'Event', 'header' => 'Event'])
@section('content')
    @php
                ...
                <div class="col-lg-8">
                    <article class="post-creative">
                        <h3 class="post-creative-title">
                            {{$event->$title_var ?? $title_var2}}
                        </h3>
                        <ul class="post-creative-meta">
                            <li>
                                <span class="icon mdi mdi-calendar-clock"></span>
                                <time datetime="{{\Carbon\Carbon::parse($event->created_at)->year}}">
                                    {{\Carbon\Carbon::parse($event->created_at)->format('M d, Y \a\t h:i A')}}
                                </time>
                            </li>
                            <li>
                        ...
                        <img class="img img-responsive" src="{{url('uploads/events/'.$event->$file_var ?? $file_var2)}}" alt=""/>

                        {!! $event->$description_var ?? $description_var2 !!}
